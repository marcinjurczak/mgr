...
var $author$project$Main$Model = F5(
	function (zone, time, weatherStatus, searchText, bookmarks) {
		return {bookmarks: bookmarks, searchText: searchText, time: time, weatherStatus: weatherStatus, zone: zone};
	});
		...
		switch (msg.$) {
			...
			case 'UpdateWeather':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{weatherStatus: $author$project$Main$Loading}),
					$author$project$Main$getWeather);
			case 'GotWeather':
				var result = msg.a;
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								weatherStatus: $author$project$Main$Success(weather)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{weatherStatus: $author$project$Main$Failure}),
						$elm$core$Platform$Cmd$none);
				}
			case 'UpdateField':
...
var $author$project$Main$viewTime = function (model) {
	var second = A2($elm$time$Time$toSecond, model.zone, model.time);
