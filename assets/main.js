...
var $author$project$Main$Model = F5(
	function (status, zone, time, text, bookmarks) {
		return {R: bookmarks, A: status, P: text, C: time, G: zone};
	});
	...
	function (msg, model) {
		switch (msg.$) {
				...
				if (!result.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								A: $author$project$Main$Success(weather)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
			...
			default:
				return _Utils_Tuple2(
					model,
					$elm$browser$Browser$Navigation$load('https://google.com/search?q=' + model.P));
		}
	});
...
var $author$project$Main$viewTime = function (model) {
	var second = A2($elm$time$Time$toSecond, model.G, model.C);
	var minute = A2($elm$time$Time$toMinute, model.G, model.C);
	var hour = A2($elm$time$Time$toHour, model.G, model.C);
