...
{
	if (region.P.C === region.W.C)
	{
		return 'on line ' + region.P.C;
	}
	return 'on lines ' + region.P.C + ' through ' + region.W.C;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aL, request.I, true);
		} catch (e) {
...
var $author$project$Main$ClockTime = F2(
	function (zone, time) {
		return {m: time, o: zone};
	});
var $author$project$Main$Loading = {$: 1};
var $author$project$Main$Model = F4(
	function (clockTime, weatherStatus, searchText, bookmarks) {
		return {S: bookmarks, t: clockTime, N: searchText, H: weatherStatus};
	});
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $author$project$Main$GotWeather = function (a) {
	return {$: 3, a: a};
};
var $author$project$Config$apiKey = 'ad058b50a02f29f63724fade627da689';
var $author$project$Config$city = 'Gdansk';
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{aA: false, aC: r.aC, aG: r.aG, Z: r.Z, aL: r.aL, aV: r.aV, au: r.au, I: r.I}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{aC: $elm$http$Http$emptyBody, aG: r.aG, Z: _List_Nil, aL: 'GET', aV: $elm$core$Maybe$Nothing, au: $elm$core$Maybe$Nothing, I: r.I});
};
var $author$project$Config$Celsius = 0;
var $author$project$Config$unitForTemp = 0;
var $author$project$Config$unit = function () {
	var _v0 = $author$project$Config$unitForTemp;
	switch (_v0) {
		case 0:
			return 'metric';
		case 1:
...
var $author$project$Config$weatherApi = 'https://api.openweathermap.org/data/2.5/weather?';
var $author$project$Main$Weather = F2(
	function (description, temperature) {
		return {V: description, at: temperature};
	});
...
var $author$project$Main$getWeather = $elm$http$Http$get(
	{
		aG: A2($elm$http$Http$expectJson, $author$project$Main$GotWeather, $author$project$Main$weatherDecoder),
		I: $author$project$Config$weatherApi + (('&q=' + $author$project$Config$city) + (('&units=' + $author$project$Config$unit) + ('&appid=' + $author$project$Config$apiKey)))
	});
...
var $author$project$Main$init = function (bookmarks) {
	return _Utils_Tuple2(
		A4(
			$author$project$Main$Model,
			A2(
				$author$project$Main$ClockTime,
				$elm$time$Time$utc,
...
var $elm$time$Time$State = F2(
	function (taggers, processes) {
		return {ai: processes, as: taggers};
	});
...
var $author$project$Main$subscriptions = function (_v0) {
	return A2($elm$time$Time$every, 10, $author$project$Main$Tick);
};
	...
	function (msg, model) {
		switch (msg.$) {
			case 0:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							t: A2($author$project$Main$ClockTime, model.t.o, newTime)
						}),
			...
			case 1:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							t: A2($author$project$Main$ClockTime, newZone, model.t.m)
						}),
					$elm$core$Platform$Cmd$none);
			case 2:
				return _Utils_Tuple2(
				...
				if (!result.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								H: $author$project$Main$Success(weather)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								H: $author$project$Main$Failure('Error: Couldn\'t retrieve weather data')
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 4:
			...
			default:
				return _Utils_Tuple2(
					model,
					$elm$browser$Browser$Navigation$load('https://google.com/search?q=' + model.N));
		}
	});
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('bookmark'),
						$elm$html$Html$Attributes$href(bookmark.I)
					]),
				_List_fromArray(
					[
						$elm$html$Html$text(bookmark.K)
					]))
			]));
};
...
var $author$project$Main$toEnglishMonth = function (month) {
	switch (month) {
		case 0:
			return 'January';
		case 1:
...
var $author$project$Main$toEnglishWeekday = function (weekday) {
	switch (weekday) {
		case 0:
			return 'Monday';
		case 1:
...
var $author$project$Main$viewDate = function (clockTime) {
	var year = A2($elm$time$Time$toYear, clockTime.o, clockTime.m);
	var weekday = A2($elm$time$Time$toWeekday, clockTime.o, clockTime.m);
	var month = A2($elm$time$Time$toMonth, clockTime.o, clockTime.m);
	var day = A2($elm$time$Time$toDay, clockTime.o, clockTime.m);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$elm$html$Html$text(
				$author$project$Main$toEnglishWeekday(weekday) + (', ' + ($elm$core$String$fromInt(day) + (' ' + ($author$project$Main$toEnglishMonth(month) + ('  ' + $elm$core$String$fromInt(year)))))))
			]));
};
var $author$project$Main$Search = {$: 5};
...
var $author$project$Main$viewSearchBar = A2(
	$elm$html$Html$div,
	_List_fromArray(
		[
		...
		[
			...
			_List_fromArray(
				[
					...
					$author$project$Main$onEnter($author$project$Main$Search)
				]),
			_List_Nil)
		]));
...
var $author$project$Main$viewTime = function (clockTime) {
	var second = A2($elm$time$Time$toSecond, clockTime.o, clockTime.m);
	var minute = A2($elm$time$Time$toMinute, clockTime.o, clockTime.m);
	var hour = A2($elm$time$Time$toHour, clockTime.o, clockTime.m);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('clock')
			]),
		_List_fromArray(
			[
				$elm$html$Html$text(
				$author$project$Main$parseTime(hour) + (':' + ($author$project$Main$parseTime(minute) + (':' + $author$project$Main$parseTime(second)))))
			]));
};
...
var $author$project$Main$getTemp = function (weather) {
	return $elm$core$String$fromInt(
		$elm$core$Basics$round(weather.at)) + ' °C ';
};
var $author$project$Main$viewWeatherStatus = function (weatherStatus) {
	switch (weatherStatus.$) {
		...
		default:
			...
			return $elm$html$Html$text(
				$author$project$Main$getDesc(weather) + (' | ' + $author$project$Main$getTemp(weather)));
	}
};
var $author$project$Main$viewWeather = function (weatherStatus) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$id('weather-description'),
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$author$project$Main$viewWeatherStatus(weatherStatus)
									]))
							]))
					]))
			]));
	...
	return {
		aC: _List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$Main$viewTime(model.t),
						$author$project$Main$viewDate(model.t),
						$author$project$Main$viewWeather(model.H),
						$author$project$Main$viewSearchBar,
						$author$project$Main$viewBookmarks(model.S)
					]))
			]),
		aW: 'Startpage'
	};
...
var $author$project$Main$main = $elm$browser$Browser$document(
	{aK: $author$project$Main$init, aU: $author$project$Main$subscriptions, aX: $author$project$Main$update, aY: $author$project$Main$view});
