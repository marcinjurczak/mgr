...
{
	if (region.O.z === region.V.z)
	{
		return 'on line ' + region.O.z;
	}
	return 'on lines ' + region.O.z + ' through ' + region.V.z;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aK, request.H, true);
		} catch (e) {
...
var $author$project$Main$Model = F5(
	function (zone, time, weatherStatus, searchText, bookmarks) {
		return {R: bookmarks, M: searchText, B: time, F: weatherStatus, G: zone};
	});
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{az: false, aB: r.aB, aF: r.aF, Y: r.Y, aK: r.aK, aU: r.aU, at: r.at, H: r.H}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{aB: $elm$http$Http$emptyBody, aF: r.aF, Y: _List_Nil, aK: 'GET', aU: $elm$core$Maybe$Nothing, at: $elm$core$Maybe$Nothing, H: r.H});
};
var $author$project$Main$Weather = F2(
	function (description, temperature) {
		return {U: description, as: temperature};
	});
...
var $author$project$Main$getWeather = $elm$http$Http$get(
	{
		aF: A2($elm$http$Http$expectJson, $author$project$Main$GotWeather, $author$project$Main$weatherDecoder),
		H: 'https://api.openweathermap.org/data/2.5/weather?appid=ad058b50a02f29f63724fade627da689&q=Gdansk&units=metric'
	});
...
var $elm$time$Time$State = F2(
	function (taggers, processes) {
		return {ah: processes, ar: taggers};
	});
	...
	function (msg, model) {
		switch (msg.$) {
				...
				if (!result.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								F: $author$project$Main$Success(weather)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
			...
			default:
				return _Utils_Tuple2(
					model,
					$elm$browser$Browser$Navigation$load('https://google.com/search?q=' + model.M));
		}
	});
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('bookmark'),
						$elm$html$Html$Attributes$href(bookmark.H)
					]),
				_List_fromArray(
					[
						$elm$html$Html$text(bookmark.J)
					]))
			]));
};
...
var $author$project$Main$viewTime = function (model) {
	var second = A2($elm$time$Time$toSecond, model.G, model.B);
	var minute = A2($elm$time$Time$toMinute, model.G, model.B);
	var hour = A2($elm$time$Time$toHour, model.G, model.B);
...
var $author$project$Main$getTemp = function (weather) {
	return $elm$core$String$fromInt(
		$elm$core$Basics$round(weather.as)) + ' °C ';
};
									...
									[
										...
										_List_fromArray(
											[
												...
												_List_fromArray(
													[
														$author$project$Main$viewWeather(model)
													]))
											]))
									]))
							])),
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$author$project$Main$viewBookmarks(model.R)
											]))
									]))
							]))
					]))
...
var $author$project$Main$main = $elm$browser$Browser$document(
	{aJ: $author$project$Main$init, aT: $author$project$Main$subscriptions, aW: $author$project$Main$update, aX: $author$project$Main$view});
